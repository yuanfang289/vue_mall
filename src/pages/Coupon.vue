<template>
  <div :class="{'coupon':'1','nostart':status == 2}" :style="{backgroundColor:bgColor}">
	<div class="bg">
		<img v-if="couponInfo.activity" :src="couponInfo.activity.cover" alt="bg"/>
	...
	<div class="list">
		...
		<template v-else v-for="item in list">
			<div :class="{'item':'1','disabled':status==4||item.status==3||item.status==4||item.status==5}">
				<div class="cou_info">
					...
					<div class="info">
						<div class="price van-ellipsis">
							<div class="rel_price van-ellipsis" v-bind:class="{ fs26: item.coupon_value>999, 'fs30': item.coupon_value<1000 }">
								<span>¥</span>{{item.coupon_value}}
							</div>
							<div class="mj van-ellipsis">
								  {{item.use_threshold==0?"满任意金额":'满'+item.use_threshold+'元可用'}}
							</div>
						</div>
						<div class="expl">
				...
				<ell-more v-bind:value='item.rule_desc' v-bind:value1='item.usage_desc' v-bind:bgColor='bgColor'></ell-more>
			</div>
		</template>
	</div>
	<div v-if="list.length>0" class="rule" :style="{color:fontColor}">
		<p class="rule_tit">规则说明</p>
		<p class="rule_description">{{couponInfo.activity.desc}}</p>
	</div>
	<div class="kong"></div>
	<div class="footer_fixed">
		<div class="btnbox">
				<template v-if="status==1">
					<div v-if="list.length==0" class="butn disabled">
						无可领优惠券
					</div>
				...
				<template v-if="status==4">
					...
					<div v-else class="butn disabled">
						券已领光
					</div>
				</template>
				
				<div v-if="status==5" class="butn disabled">
					已领取
				</div>
		</div>
	</div>
...
export default {
  ...
  methods: {
	...
	getCouponInfo(){
	    let _ = this, url ='/index/coupon/coupon_list?id='+_.id+'&sell_id='+_.sell_id;
		if(_.token) { url = '/index/coupon/coupon_list?id='+_.id+'&sell_id='+_.sell_id+'&token='+_.token}
	    _.$axios.get(url).then(
	  	 function (res) {
			 ...
			 if(_.list.length>0&&a.length==0){
				 _.status = 5;
			 }
			 
			 if(_.status == 3||_.status == 4){
				if(_.status == 3){_.$toast.clear();_.receiveFailPopup('您来晚了，活动已结束～')};
				if(_.status == 4){_.$toast.clear();_.receiveFailPopup('您来晚了，券领光了～')};
			 }else{
			 	if(_.list.length == 0){
					_.$toast.clear();
	...
	gotoUse(e){
	  ...
	  _.sensors.track('ClickButton', {
	    'event_name': '点击去使用优惠券',
		'current_page':'领券页面',
	    'new_coupon_id': e.coupon_id,
	    'button_name': '去使用',
		'activity_id': _.couponInfo.activity.id+'',
		'coupon_code':e.coupon_member_code+''
	  });
	  
	  let data = Object.assign({scence:scence,coupon_code:e.coupon_member_code,sell_id:_.sell_id},_.tokenObj)
	  _.$axios.post('/index/coupon_code/goto_use_code',data).then(
	  	function (res) {
			if(res.data.code){
				let url = res.data.data.jump_path;
				_.navigateTo('/'+url,url,e.coupon_member_code)
			}else{
	...
	// 跳转
	navigateTo(appurl,url,code){
		let _ = this;
		if(_.platform === 'miniprogram'){
			// 小程序环境
			if(appurl.indexOf('/h5/shop/index#/UseCoupon') != -1){
				// appurl =  '/pages/ui/webview?couponCode='+code+'&url='+appurl
				window.location.href = url+'&from=mini'
			}else{
