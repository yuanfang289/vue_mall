<template>
	<div class="cou_ex" ref="cou_ex" @click.capture=" is_hide_more ? '' : wrapper($event)">
		...
		<div ref="tips" class="tips"  style="opacity:0;">
		...
		<div v-if="!is_hide_more" class="icon">
		</div>
...
export default {
  ...
  watch: {
      value (newV, oldV) { // watch监听props里数据的变化，然后执行操作
          let _ = this;
      },
  },
  created() {
          let _ = this;
		  _.$nextTick(function(){
				let a = _.$refs.tips.scrollHeight;
				let b = _.$refs.cou_ex.scrollHeight;
				if((a-b)<10){
					_.is_hide_more = true
				}
		  })
